...
\usepackage{sty/Rd}
\usepackage{sty/Sweave}
\usepackage{sty/upquote}
...
\newcommand{\ac}{\textsc{AutoCast}}

\title{\ac: Automated Bayesian Forecasting with \YourCast\thanks{Available from
    http://GKing.Harvard.Edu/autocast via a Creative Commons
    Attribution-Noncommercial-No Derivative Works 3.0, for academic
    use only.}}

\author{Jonathan Bischof\thanks{Department of Statistics, Harvard
...
\abstract{\ac\ is a streamlined and user-friendly version of
  ...
  set.  \ac\ introduces a framework that makes setting multiple
...
In this paper we introduce \ac, a streamlined version of the
...
\ac\ allows analysts to disentangle the interactions between
...
from the calculation.  The four components are:
\begin{itemize}
\item \emph{Prediction error} is measured as sum of the root mean square error of the
  out-of-sample forecasts for all of the omitted years in all
  groups.
...
for a more direct optimization method such as BFGS.  \ac\ provides
...
\ac\ uses much of the basic architecture of \YourCast, including how
...
In order to illustrate how functions in \ac\ are used, we will focus
...
forming an objective function using \ac\, we will be able to find
optimal forecasts given our personal preferences for different
optimality criteria.

\subsection{Grid search}
Once we decide on a set of weights for our objective function, \ac\
offers two optimization methods: a grid search function called
\texttt{autocast} and a general purpose optimization function
called \texttt{optim.autocast} that can call \texttt{optim} or
...
with \texttt{autocast} and then using the minimum from the grid
search as a starting point for one of the optimization algorithms
implemented in \texttt{optim.autocast}. Since the objective function
surface is almost never unimodal, a thorough grid search is critical.

We start with the grid search function \texttt{autocast}. We
...
Since \texttt{autocast} works through cross-validation, we also
...
omitting a block of observations at the end is the only meaningful
...
\texttt{plot.autocast} can generate plots for the individual
...
\texttt{autocast} output object with the \texttt{summary}
command:
...
Similar output is generated when \texttt{summary} is used on a \texttt{optim.autocast} output object.


\subsection{Direct optimization}
\ac\ includes the function \texttt{optim.autocast}, which can
directly optimize the objective function by making calls to
\texttt{optim} or \texttt{rgenoud}. The arguments to
\texttt{optim.autocast} are almost identical to
\texttt{autocast} except that the user must specify the type of
optimization to employ rather than the grid points.

However, direct optimization rarely works without a good starting
point provided by a grid search. \ac\ makes it very easy to pass the
result of a grid search on to an optimizer with the call:

\begin{verbatim}
out.opt <- optim.autocast(out.auto)
\end{verbatim}
Here \texttt{optim.autocast} will automatically transfer all relevant
information about the \texttt{yourcast} model from the
\texttt{autocast} output object. Optimization will be started
...
Once an initial grid search is completed, \ac\ allows users to
...
another call to \code{autocast} specifying only the
...
\end{verbatim}
If desired, each \texttt{autocast} output can also be further refined by \texttt{optim.autocast} before being sent to \texttt{run.opt}.

The plots discussed in this section are displayed in Figure \ref{forecasts}.

\section{Reference}

The following pages list the main functions in \ac\ with
detailed reference information.  These can also be loaded from \R\ 
with the standard help command, such as \code{help(autocast)}.

\include{Rd/autocast}
\include{Rd/optim.autocast}
\include{Rd/plot.autocast}
\include{Rd/run.opt}
\include{Rd/plot.diag}

\bibliographystyle{apsr}
\bibsep=0in
\bibliography{autocast}
